...
export default function BuyerOrdersPage() {
  ...
  const loadOrders = async () => {
    if (!user) return;
    try {
      // Only fetch transactions where this user is the buyer
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
        ...
        <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-12 flex items-center justify-center">
          <div className="bg-white border border-slate-200 rounded-3xl p-10 text-center space-y-4 shadow-xs max-w-md w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
              <ShoppingBag className="w-6 h-6 text-blue-600" /> My Buyer Orders
            </h1>
          ...
          {orders.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-3xl p-10 text-center space-y-4 shadow-xs">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-[#006B3F] hover:bg-[#005432] text-white font-bold text-xs transition-all shadow-md"
              >
            ...
            orders.map((o) => (
              <div
                key={o.id}
                className="bg-white border border-slate-200 rounded-3xl p-5 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:shadow-sm transition-shadow"
              >
                <div className="space-y-1.5 flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-mono text-[#006B3F] font-bold text-xs">{o.code}</span>
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase border flex items-center gap-1 ${STATE_COLORS[o.state] || 'bg-slate-100 text-slate-700 border-slate-200'}`}
                    >
                      {STATE_ICONS[o.state]} {o.state}
                    </span>
                  </div>
                  <h3 className="font-bold text-slate-900 text-sm truncate">{o.title}</h3>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto shrink-0">
                  <Link
                    ...
                    className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#006B3F] hover:bg-[#005432] text-white font-bold text-xs text-center transition-all flex items-center justify-center gap-1.5 shadow-xs"
                  >
